        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                ...
                if button_status.hit and button_collide_instance.hit_button_area.\
                    ...
                    if dbq == "S":
                        message4 = common_vars.text2_font.render('You Should STAND Here ', False, RED_COLOR)
                        pygame.display.update(common_vars.screen.blit(message4, (x_pos, y_pos + 100)))
                        pygame.time.delay(2000)
                    elif dbq == "H":
                        message4 = common_vars.text2_font.render('You Should HIT Here  ', False, RED_COLOR)
                        pygame.display.update(common_vars.screen.blit(message4, (x_pos, y_pos + 100)))
                        pygame.time.delay(2000)
                    elif dbq == "P":
                        message4 = common_vars.text2_font.render('You Should SPLIT Here  ', False, RED_COLOR)
                        pygame.display.update(common_vars.screen.blit(message4, (x_pos, y_pos + 100)))
                        pygame.time.delay(2000)
                    elif dbq == "D":
                        message4 = common_vars.text2_font.render('You Should DOUBLE DOWN Here ', False, RED_COLOR)
                        pygame.display.update(common_vars.screen.blit(message4, (x_pos, y_pos + 100)))
                        pygame.time.delay(2000)
                    else:
                        continue
    ...
    def __call__(self, common_vars, button_status):
        ...
        if value_of_dealer_hand == 21:
            ...
            if value_of_player_hand < 21:
                # Player's current hand lose against dealer
                common_vars.pause_time = PAUSE_TIMER3
                plot_results(common_vars.screen, common_vars.text_font,
                             'Dealer has {0}, Player has {1}'.format(value_of_dealer_hand, value_of_player_hand))
                if num_of_hands == 1:
                    # Only one player hand
                    common_vars.hands_status['first_hand_lose'] = True
                    self._current_hand = 0
                    button_status.reset()
                    self.next_state(InitialState)
                    ...
                    common_vars.hands_status['second_hand_lose'] = True
                    self._current_hand = 0
                    button_status.reset()
                    self.next_state(InitialState)
            else:
